...
# Finds if the filename contains a Visit number, returns what it should call the Visit folder
def get_visitFolder(filename):
    pattern = r'[Vv][0-9]+'
    match = re.findall(pattern, filename)
    if match:  # if the pattern is found, then....
        stop = re.search(pattern, filename)
        folder = filename[0:stop.span()[1]] # get the name up untill the end of the visit , return that as the folder name
        return folder
    else:
        folder = filename[0:8]  # else just return the folder name using just the subject's studyID as the name for the folder
        return folder
